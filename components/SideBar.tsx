...
function dropDown(name: String, icon: React.JSX.Element, contents: React.JSX.Element) {
    ...
    return <Accordion>
        ...
        >
            {icon}
            <Typography
                className={" max-w-60 px-2"}
                noWrap fontWeight='600' >{name} </Typography>
        </AccordionSummary>
...
const priceRange = (
    <Box className='flex shadow px-4 py-2 min-h-max items-center'>
        <SellIcon />
        <Typography className={"min-w-max px-2"}
        ...
        <TextField
            ...
            size="small"
            type="number" />
        <Typography className={"min-w-max px-1"}
            noWrap fontWeight='600' >
            –
        </Typography>
        <TextField
    ...
    return (
        <Box className='grid content-between w-80 max-h-full overflow-y-auto'
            ...
            <Box id="dropdown-Menus" className='w-auto '>
                ...
                {dropDown("Condition: New/Used", <BarChartIcon />, conditionDropdownContent)}
